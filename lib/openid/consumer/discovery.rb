...
module OpenID
  ...
  def self.discover_xri(iname, xri_proxy)
    endpoints = []
    iname = self.normalize_xri(iname)

    begin
      canonical_id, services = Yadis::XRI::ProxyResolver.new(xri_proxy).query( iname )
  ...
  def self.discover(identifier, xri_proxy = nil)
    if Yadis::XRI::identifier_scheme(identifier) == :xri
      normalized_identifier, services = discover_xri(identifier, xri_proxy)
    else
